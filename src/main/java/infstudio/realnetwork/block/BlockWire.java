...
public class BlockWire extends BlockWireBase {
    ...
    public void addCollisionBoxToList(IBlockState state, World worldIn, BlockPos pos, AxisAlignedBB entityBox, List<AxisAlignedBB> collidingBoxes, @Nullable Entity entityIn, boolean isActualState) {
        ...
        if (canConnect(worldIn, pos, pos.north(), EnumFacing.NORTH)) addCollisionBoxToList(pos, entityBox, collidingBoxes, NORTH_AABB);
        if (canConnect(worldIn, pos, pos.south(), EnumFacing.SOUTH)) addCollisionBoxToList(pos, entityBox, collidingBoxes, SOUTH_AABB);
        if (canConnect(worldIn, pos, pos.east(), EnumFacing.EAST)) addCollisionBoxToList(pos, entityBox, collidingBoxes, EAST_AABB);
        if (canConnect(worldIn, pos, pos.west(), EnumFacing.WEST)) addCollisionBoxToList(pos, entityBox, collidingBoxes, WEST_AABB);
        if (canConnect(worldIn, pos, pos.up(), EnumFacing.UP)) addCollisionBoxToList(pos, entityBox, collidingBoxes, UP_AABB);
        if (canConnect(worldIn, pos, pos.down(), EnumFacing.DOWN)) addCollisionBoxToList(pos, entityBox, collidingBoxes, DOWN_AABB);
    }
