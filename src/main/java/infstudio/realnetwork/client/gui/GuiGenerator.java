...
import infstudio.realnetwork.inventory.ContainerGenerator;
...
public class GuiGenerator extends GuiMachineBase {
    ...
    public GuiGenerator(ContainerGenerator inventorySlotsIn) {
        ...
        this.texture = new ResourceLocation(RealNetwork.MODID+":textures/gui/container/generator.png");
        this.frontTexture = new ResourceLocation(RealNetwork.MODID+":textures/blocks/machine/generator_front.png");
    ...
    protected void drawGuiContainerForegroundLayer(int mouseX, int mouseY) {
        ...
        String emf = String.format("Em=%.1fV", 1.0*container.getE()/10);
        this.fontRenderer.drawString(emf, 100, 36, 4210752);
    }
    ...
    protected void actionPerformed(GuiButton button) throws IOException {
        ...
        for (int i = 0; i < 4; ++i) {
            if (button.id == buttonInc[i]) {
                int Enew = container.getE()+(int)Math.pow(10, i);
                ((TileEntityGenerator)container.getTileMachine()).setE(1.0*Enew/10, 0);
